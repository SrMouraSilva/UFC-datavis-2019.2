// https://observablehq.com/@srmourasilva/d3-com-crossfilter-e-dc-js-parte-2@241
  ...
  main.variable(observer("buildvis")).define("buildvis", ["md","container","crossfilter","dataset","d3","dc"], function(md,container,crossfilter,dataset,d3,dc)
{
  ...
  const dateDim = facts.dimension(d => d.dtg)
  const magnitudeDim = facts.dimension(d => d.magnitude)
  const depthDim = facts.dimension(d => d.depth)
  const hourDim = facts.dimension(d => d3.timeHour(d.dtg))
  
  const table = dc.dataTable(view.querySelector("#dc-table-graph"))
  const magnitudeChart = dc.barChart(view.querySelector("#magnitude-chart"))
  const depthChart = dc.barChart(view.querySelector("#depth-chart"))
  const timeChart = dc.lineChart(view.querySelector("#time-chart"))
